...
class ViewController: UIViewController, DataUpdateProtocol {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "toEditScreen":
            prepareEditScreem(segue)
        default:
            break
        }
    }
    
    private func prepareEditScreem(_ segue: UIStoryboardSegue){
    ...
    @IBAction func editDataWithClouser(_ sender: UIButton){
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let editScreen = storyboard.instantiateViewController(withIdentifier: "SecondViewController") as! SecondViewController
        
        editScreen.updatingData = dataLabel.text ?? ""
        // передаем необходимое замыкание
        //[unowned self] указывает на то, что замыкание будет захватывать self без удержания его в памяти. Это делается для предотвращения утечек памяти.
        editScreen.completionHandler = {[unowned  self] updateValue in
            updatedData = updateValue
            updateLabel(withText: updateValue)
        }
        self.navigationController?.pushViewController(editScreen, animated: true)
    }
}
